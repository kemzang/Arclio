...
export const ar = {
  ...
  feat_workflow_1:
    "**ألصق أي رابط** — فيديوهات YouTube وShorts والقنوات وقوائم التشغيل والبودكاست والموسيقى، إضافةً إلى 2000+ موقع آخر يدعمها yt-dlp؛ نزّل قائمة التشغيل كاملة أو اختر فيديوهات محددة أولاً",
  feat_workflow_2:
  ...
  feat_workflow_8:
    "**الوضع الحذر** — إعدادات وتيرة قابلة للتخصيص (*إيقاف · متوازن · حذر · مخصص*) تضيف فواصل بين الطلبات وتحد من خيوط الأجزاء، مما يقلل احتمال حظر البوتات في قوائم التشغيل الكبيرة",
  ...
  faq_a8:
    "نعم — كلاهما. الصق رابط قائمة تشغيل أو رابط قناة (مثل `youtube.com/@handle`، `/channel/UC…`، `/c/Name`، `/user/Old`)؛ يُعدِّد Arroxy حتى 500 عنصر، ثم تضع القائمة كاملة أو فيديوهات محددة في قائمة الانتظار. فلاتر نطاق التاريخ والعدد ستأتي قريباً.",
  ...
  plan_r1_desc:
    "فلاتر نطاق التاريخ والعدد عند تعداد قائمة تشغيل أو قناة (الحد الآن ثابت عند 500 عنصر)",
