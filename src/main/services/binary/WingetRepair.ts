...
export async function installYtDlpWithWinget(): Promise<string> {
	...
	try {
		await execFileAsync(wingetPath, ['install', '--id', 'yt-dlp.yt-dlp', '--exact', '--silent', '--accept-package-agreements', '--accept-source-agreements'], {timeout: WINGET_INSTALL_TIMEOUT_MS, maxBuffer: 4 * 1024 * 1024, windowsHide: true})
	} catch (error) {
