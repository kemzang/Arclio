...
export function playlistScopeSentinelFields(scope: PlaylistScope | undefined, appLimit: number): PlaylistScopeSentinelFields {
	...
	if (!items) {
		return {requestedCount: appLimit, sentinel: true, ytDlpFlag: '--playlist-end', ytDlpValue: String(appLimit + 1)}
	}
	...
	switch (items.kind) {
		case 'app-limit':
			return {requestedCount: appLimit, sentinel: true, ytDlpFlag: '--playlist-end', ytDlpValue: String(appLimit + 1)}
		case 'first':
			return {requestedCount: items.count, sentinel: true, ytDlpFlag: '--playlist-items', ytDlpValue: `1:${items.count + 1}`}
		case 'range':
			return {requestedCount: items.to - items.from + 1, sentinel: true, ytDlpFlag: '--playlist-items', ytDlpValue: `${items.from}:${items.to + 1}`}
		default:
