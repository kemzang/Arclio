const ja = {
	common: {back: '戻る', cancel: 'キャンセル', continue: '続行', retry: '再試行', startOver: '最初からやり直す'},
	app: {feedback: 'フィードバック', logs: 'ログ', feedbackNudge: 'Arroxyを楽しんでくれてる?ぜひ感想を聞かせて! 💬', debugCopied: 'コピーしました!', debugCopyTitle: 'デバッグ情報をコピー (Electron、OS、Chrome のバージョン)', zoomIn: '拡大', zoomOut: '縮小'},
	about: {button: '情報', openTitle: 'Arroxy について', tagline: 'デスクトップ向けの高速で使いやすい動画・音声ダウンローダー。', websiteLink: '公式サイト', githubLink: 'GitHub', licenseLine: 'MIT ライセンス · 作者 Antonio Orionus', thirdPartyNotices: 'サードパーティ通知を表示'},
	titleBar: {close: '閉じる', minimize: '最小化', maximize: '最大化', restore: '元に戻す'},
	splash: {greeting: 'やあ、おかえり!', warmup: 'Arroxyを起動中…', downloading: '{{binary}} をダウンロード中…', warmupFailedNoDiag: 'セットアップに失敗しました。セットアップログを開いて詳細を確認してください。'},
	repair: {
	...
	theme: {light: 'ライトモード', dark: 'ダークモード', system: 'システム設定に従う'},
	language: {label: '言語'},
	wizard: {
		steps: {url: 'URL', playlistItems: 'Playlist', playlistPresets: '画質', formats: '形式', subtitles: '字幕', sponsorblock: 'SponsorBlock', output: '出力', folder: '保存先', confirm: '確認'},
		playlist: {
		...
		url: {
			...
			features: {
				...
				youtube: {heading: 'YouTube', video: '動画', channel: 'チャンネル', playlist: 'プレイリスト', short: 'Shorts', music: '音楽', podcast: 'ポッドキャスト'},
				anySite: {heading: '2000以上のサイト', video: '動画', videoPlaylist: '動画プレイリスト', musicPlaylist: '音楽プレイリスト'},
				always: {heading: 'いつでも利用可能', audioOnly: '音声のみ', subtitles: '字幕'}
			},
			...
			proxy: {label: 'Proxy URL', description: 'プロキシ経由でトラフィックを転送 — 地域制限コンテンツに有効。', placeholder: 'http://host:port', clear: 'クリア'},
			playlistProbeLimit: {
			...
			singleFilenameId: {toggle: '単体ファイル名に動画IDを追加', toggleDescription: 'タイトルが変わったり重複したりしても、単発ダウンロードを一意に保ちます。'},
			networkPacing: {
				heading: 'やさしいダウンロード',
				...
				presets: {off: 'オフ', balanced: 'バランス', careful: '慎重', custom: 'カスタム'},
				tooltips: {
				...
				fields: {sleepRequests: 'メタデータチェック間の待機', sleepInterval: 'メディア開始前の一時停止：最小', maxSleepInterval: 'メディア開始前の一時停止：最大', sleepSubtitles: '字幕ファイル前の待機', concurrentFragments: 'ダウンロード接続数'},
				units: {seconds: '秒', threads: 'スレッド'},
				presetLabel: 'Arroxy はどのくらい慎重にしますか？'
			},
			closeToTray: {toggle: '閉じるときにトレイに格納', toggleDescription: 'ウィンドウを閉じた後もバックグラウンドでダウンロードを続けます。'},
			analytics: {toggle: '匿名の使用統計を送信', toggleDescription: 'アプリの起動回数のみを記録します。URL、ファイル名、個人情報は含まれません。'},
			limitRate: {
				label: 'ダウンロード速度制限',
		...
		subtitles: {
			...
			sectionManual: '手動',
			sectionAuto: '自動生成',
			saveMode: {heading: '保存方法', sidecar: '動画と同じ場所', embed: '動画に埋め込む', subfolder: 'subtitles/ サブフォルダー'},
		...
		sponsorblock: {
			...
			mode: {off: 'オフ', mark: 'チャプターとしてマーク', remove: 'セグメントを削除'},
			modeHint: {off: 'SponsorBlockなし — 動画はアップロードされた通りに再生されます。', mark: 'スポンサーセグメントをチャプターとしてマークします（非破壊的）。', remove: 'FFmpegを使用してスポンサーセグメントを削除します。'},
			categoriesHeading: 'カテゴリ',
			cat: {sponsor: 'スポンサー', intro: 'イントロ', outro: 'アウトロ', selfpromo: '自己宣伝', music_offtopic: '音楽（無関係）', preview: 'プレビュー', filler: 'フィラー'}
		},
		formats: {
			...
			cookiesError: {
				...
				needsCookies: {heading: 'このサイトはサインインが必要です', body: 'yt-dlpは認証なしでこの動画にアクセスできませんでした。詳細設定でCookieを設定してください — すでにサインイン済みのブラウザを指定するか、cookies.txtファイルをインポートしてください。'},
		...
		folder: {
			...
			home: 'ホーム',
			custom: 'カスタム…',
			subfolder: {toggle: 'サブフォルダに保存', placeholder: '例: lo-fi rips', invalid: 'フォルダ名に無効な文字が含まれています'}
		},
		output: {
			embedChapters: {label: 'チャプターを埋め込む', description: '現代のプレーヤーでナビゲートできるチャプターマーカー。'},
			embedMetadata: {label: 'メタデータを埋め込む', description: 'タイトル、アーティスト、説明、アップロード日をファイルに書き込みます。'},
			embedThumbnail: {label: 'サムネイルを埋め込む', description: 'ファイル内のカバーアート。WebM動画はMKVに再パッケージされます。字幕を埋め込む場合はスキップ。'},
			writeDescription: {label: '説明を保存', description: '動画の説明を .description テキストファイルとしてダウンロードの隣に保存します。'},
			writeThumbnail: {label: 'サムネイルを保存', description: 'サムネイルを .jpg 画像ファイルとしてダウンロードの隣に保存します。'},
			writeM3u: {label: '.m3u プレイリストファイルを生成', description: '動画の横に .m3u プレイリストを保存し、メディアプレーヤーで順番に開けるようにします。'}
		},
		confirm: {
	...
	videoCard: {titlePlaceholder: '読み込み中…'},
	queue: {
		header: 'ダウンロードキュー',
	...
	presets: {
		'best-quality': {label: '最高画質', desc: '最高解像度 + 最高音質'},
		balanced: {label: 'バランス', desc: '720p最大 + 良好な音質'},
		'audio-only': {label: '音声のみ', desc: '動画なし、最高音質'},
		'small-file': {label: '小サイズ', desc: '最低解像度 + 低音質'},
		'subtitle-only': {label: '字幕のみ', desc: '動画・音声なし、字幕のみ'}
	},
	playlistPresets: {
		type: {video: 'Video', audio: 'Audio'},
		videoFormat: {best: 'Best codec', mp4: 'MP4 (H.264)'},
		videoFormatDesc: {best: '各項目で利用可能な最高のコーデック', mp4: 'H.264 + AAC を優先、MP4 コンテナ · 可能な範囲で'},
		tier: {best: 'Best quality', '2160': '最大 4K', '1440': '最大 1440p', '1080': '最大 1080p', '720': '最大 720p', '480': '最大 480p', '360': '最大 360p'},
		tierDesc: {
		...
		audioFormatDesc: {best: '最高のネイティブ音声、再エンコードなし', mp3: 'MP3 に変換', m4a: 'M4A (AAC) に変換', opus: 'Opus に変換'},
		audioFormatBitrate: 'Audio ({{format}} {{kbps}}K)',
		mp4Cap: 'YouTube では 1080p 超の H.264 は利用できません — 自動的に 1080p に制限されます'
	},
	formatLabel: {audioFallback: '音声', audioOnlyDot: '音声のみ · {{audio}}', videoDot: '{{resolution}} · {{audio}}'},
	tray: {tooltip: 'Arroxy', menu: {statusIdle: 'アイドル', statusActive_one: '1件ダウンロード中 · {{percent}}%', statusActive_other: '{{count}}件ダウンロード中 · {{percent}}%', open: 'Arroxyを開く', quit: 'Arroxyを終了'}},
	dialogs: {
		quitWithActiveDownloads: {message_one: 'ダウンロード{{count}}件を実行中', message_other: 'ダウンロード{{count}}件を実行中', detail: '閉じるとアクティブなダウンロードはすべてキャンセルされます。', confirm: 'ダウンロードをキャンセルして終了', keep: 'ダウンロードを続ける', pause: 'ダウンロードを一時停止して終了'},
		closeToTray: {message: '閉じるときにArroxyをシステムトレイに格納しますか？', detail: 'Arroxyは実行を続け、アクティブなダウンロードを完了します。詳細設定で変更できます。', hide: 'トレイに格納', quit: '終了', remember: '次回から表示しない'},
		rendererCrashed: {message: 'Arroxyで問題が発生しました', detail: 'レンダラープロセスがクラッシュしました ({{reason}})。再読み込みしてもう一度お試しください。', reload: '再読み込み', quit: '終了'}
	},
	share: {
		title: 'Arroxyをシェア',
