const en = {
	common: {back: 'Back', cancel: 'Cancel', continue: 'Continue', retry: 'Retry', startOver: 'Start over'},
	app: {feedback: 'Feedback', logs: 'Logs', feedbackNudge: "Enjoying Arroxy? I'd love to hear from you! 💬", debugCopied: 'Copied!', debugCopyTitle: 'Copy debug info (Electron, OS, Chrome versions)', zoomIn: 'Zoom in', zoomOut: 'Zoom out'},
	about: {button: 'About', openTitle: 'About Arroxy', tagline: 'Fast, friendly downloader for YouTube and 2000+ other sites.', websiteLink: 'Website', githubLink: 'GitHub', licenseLine: 'MIT License · by Antonio Orionus', thirdPartyNotices: 'View third-party notices'},
	titleBar: {close: 'Close', minimize: 'Minimize', maximize: 'Maximize', restore: 'Restore'},
	splash: {greeting: 'Hey, welcome back!', warmup: 'Arroxy is warming up…', downloading: 'Downloading {{binary}}…', warmupFailedNoDiag: 'Setup failed. Open the setup log for details.'},
	repair: {
		...
		actions: {chooseExecutable: 'Choose executable', installWithHomebrew: 'Install with Homebrew', installWithWinget: 'Install with WinGet', resetToDefault: 'Reset to default', retrySetup: 'Retry setup', cancel: 'Cancel', openDependencyFolder: 'Open dependency folder', viewSetupLog: 'View setup log'}
	},
	theme: {light: 'Light mode', dark: 'Dark mode', system: 'System default'},
	language: {label: 'Language'},
	wizard: {
		steps: {url: 'URL', playlistItems: 'Playlist', playlistPresets: 'Quality', formats: 'Format', subtitles: 'Subtitles', sponsorblock: 'SponsorBlock', output: 'Output', folder: 'Save', confirm: 'Confirm'},
		playlistPresets: {heading: 'Pick quality for the batch', subhead: 'Each video resolves the chosen tier independently — heterogeneous playlists work without surprises.', itemCount_one: '{{count}} item', itemCount_other: '{{count}} items'},
		playlist: {
		...
		url: {
			...
			proxy: {label: 'Proxy URL', description: 'Route traffic through a proxy — useful for geo-restricted content.', placeholder: 'http://host:port', clear: 'Clear'},
			limitRate: {
			...
			singleFilenameId: {toggle: 'Add video ID to single filenames', toggleDescription: 'Keeps one-off downloads unique when titles change or collide.'},
			networkPacing: {
				...
				presets: {off: 'Off', balanced: 'Balanced', careful: 'Careful', custom: 'Custom'},
				tooltips: {
				...
				fields: {sleepRequests: 'Wait between metadata checks', sleepInterval: 'Pause before media starts: min', maxSleepInterval: 'Pause before media starts: max', sleepSubtitles: 'Wait before subtitle files', concurrentFragments: 'Download connections'},
				units: {seconds: 'sec', threads: 'threads'}
			},
			closeToTray: {toggle: 'Hide to tray on close', toggleDescription: 'Continue downloads in the background after closing the window.'},
			analytics: {toggle: 'Send anonymous usage stats', toggleDescription: 'Counts app launches only. No URLs, filenames, or personal data.'}
		},
		subtitles: {
			autoBadge: 'Auto',
			...
			saveMode: {heading: 'Save as', sidecar: 'Next to video', embed: 'Embed into video', subfolder: 'subtitles/ subfolder'},
		...
		sponsorblock: {
			...
			mode: {off: 'Off', mark: 'Mark as chapters', remove: 'Remove segments'},
			modeHint: {off: 'No SponsorBlock — video plays as uploaded.', mark: 'Marks sponsor segments as chapters (non-destructive).', remove: 'Cuts sponsor segments from the video using FFmpeg.'},
		...
		formats: {
			...
			cookiesError: {
				...
				needsCookies: {heading: 'This site requires sign-in', body: "yt-dlp couldn't access this video without authentication. Configure cookies in advanced settings — point them at a browser you're already signed in to, or import a cookies.txt file."},
				dpapi: {
		...
		folder: {heading: 'Save to', downloads: 'Downloads', videos: 'Movies', desktop: 'Desktop', music: 'Music', documents: 'Documents', pictures: 'Pictures', home: 'Home', custom: 'Custom…', subfolder: {toggle: 'Save inside subfolder', placeholder: 'e.g. lo-fi rips', invalid: 'Folder name has invalid characters'}},
		output: {
			embedChapters: {label: 'Embed chapters', description: 'Chapter markers navigable in any modern player.'},
			embedMetadata: {label: 'Embed metadata', description: 'Title, artist, description, and upload date written into the file.'},
			embedThumbnail: {label: 'Embed thumbnail', description: 'Cover art inside the file. WebM video will be remuxed to MKV; skipped when subtitles are embedded.'},
			writeDescription: {label: 'Save description', description: 'Saves the video description as a .description text file next to the download.'},
			writeThumbnail: {label: 'Save thumbnail', description: 'Saves the thumbnail as a .jpg image file next to the download.'},
			writeM3u: {label: 'Generate .m3u playlist file', description: 'Save an .m3u playlist alongside the videos so they open in order in your media player.'}
		},
		confirm: {
	...
	videoCard: {titlePlaceholder: 'Loading…'},
	queue: {
	...
	presets: {
		'best-quality': {label: 'Best quality', desc: 'Highest resolution + best audio'},
		balanced: {label: 'Balanced', desc: '720p max + good audio'},
		'audio-only': {label: 'Audio only', desc: 'No video, best audio'},
		'small-file': {label: 'Small file', desc: 'Lowest resolution + low audio'},
		'subtitle-only': {label: 'Subtitles only', desc: 'No video, no audio, only subtitles'}
	},
	playlistPresets: {
		type: {video: 'Video', audio: 'Audio'},
		videoFormat: {best: 'Best native', mp4: 'MP4 / Smart TV'},
		videoFormatDesc: {best: 'Best available container and codecs per item', mp4: 'Prefer H.264 video + AAC audio in MP4 when possible'},
		tier: {best: 'Best quality', '2160': 'Up to 4K', '1440': 'Up to 1440p', '1080': 'Up to 1080p', '720': 'Up to 720p', '480': 'Up to 480p', '360': 'Up to 360p'},
		tierDesc: {
		...
		audioFormatDesc: {best: 'Native best audio, no re-encode', mp3: 'Convert to MP3', m4a: 'Convert to M4A (AAC)', opus: 'Convert to Opus'},
		audioFormatBitrate: 'Audio ({{format}} {{kbps}}K)',
		mp4Cap: 'H.264 above 1080p is not available on YouTube — capped to 1080p automatically'
	},
	formatLabel: {audioFallback: 'Audio', audioOnlyDot: 'Audio only · {{audio}}', videoDot: '{{resolution}} · {{audio}}'},
	tray: {tooltip: 'Arroxy', menu: {statusIdle: 'Idle', statusActive_one: '1 downloading · {{percent}}%', statusActive_other: '{{count}} downloading · {{percent}}%', open: 'Open Arroxy', quit: 'Quit Arroxy'}},
	dialogs: {
		quitWithActiveDownloads: {message_one: '{{count}} download in progress', message_other: '{{count}} downloads in progress', detail: 'Pause to resume later, or cancel to discard progress.', confirm: 'Cancel Downloads & Quit', pause: 'Pause Downloads & Quit', keep: 'Keep Downloading'},
		closeToTray: {message: 'Hide Arroxy to the system tray when closing?', detail: 'Arroxy keeps running and finishes active downloads. Change this later in Advanced settings.', hide: 'Hide to tray', quit: 'Quit', remember: "Don't ask again"},
		rendererCrashed: {message: 'Arroxy encountered a problem', detail: 'The renderer process crashed ({{reason}}). Reload to try again.', reload: 'Reload', quit: 'Quit'}
	},
	share: {
		title: 'Share Arroxy',
