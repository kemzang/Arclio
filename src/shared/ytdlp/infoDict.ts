...
export const ytDlpSubtitleTrackSchema = z
  .object({
    ext: optStr,
    name: optStr,
    url: optStr,
...
export const infoDictSchema: z.ZodType<InfoDict> = z.lazy(() => {
  const videoArm = videoFieldsSchema
    .extend({
      _type: z.preprocess((v) => v ?? 'video', z.literal('video'))
    })
    .loose();

  const playlistArm = playlistFieldsSchema
    .extend({
      _type: z.literal('playlist'),
      entries: z.array(infoDictSchema)
    })
    .loose();

  const multiVideoArm = playlistFieldsSchema
    .extend({
      _type: z.literal('multi_video'),
      entries: z.array(infoDictSchema)
    })
    .loose();

  const urlArm = videoFieldsSchema
    .extend({
      _type: z.literal('url'),
      url: z.string(),
      ie_key: optStr
  ...
  const urlTransparentArm = videoFieldsSchema
    .extend({
      _type: z.literal('url_transparent'),
      url: z.string(),
      ie_key: optStr
    })
    .loose();
