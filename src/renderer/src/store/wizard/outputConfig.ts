...
export function createOutputConfigSlice(set: SetState, get: GetState): OutputConfigSlice {
	return {
		...
		toggleSponsorBlockCategory: cat => set(state => ({wizardSponsorBlockCategories: state.wizardSponsorBlockCategories.includes(cat) ? state.wizardSponsorBlockCategories.filter(c => c !== cat) : [...state.wizardSponsorBlockCategories, cat]})),
