...
export function bulkStressFixture(count = 50): BulkStressFixture {
  ...
  const entries: PlaylistEntry[] = Array.from({ length: count }, (_, i) => {
    ...
    return {
      id: `bulk-stress-${number}`,
      url: `https://www.youtube.com/watch?v=bulk${String(number).padStart(8, '0')}`,
      title,
      thumbnail: i % 4 === 0 ? '' : 'https://i.ytimg.com/vi/jfKfPfyJRdk/mqdefault.jpg',
      duration: i % 6 === 0 ? undefined : 120 + i * 33,
  ...
  return {
    entries,
    metadataById,
    completed: Object.values(metadataById).filter((status) => status !== 'pending' && status !== 'resolving').length,
