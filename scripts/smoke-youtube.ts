...
async function runStrategy(name: string, binary: string, url: string, extractorArgs: string | null, cookiesPath?: string): Promise<StrategyReport> {
	...
	if (r.exitCode !== 0) {
		return {name, passed: false, durationMs: r.durationMs, error: r.stderr.trim().split('\n').pop() ?? `exit ${r.exitCode}`}
	}
	...
	try {
		...
		return {name, passed: false, durationMs: r.durationMs, error: `JSON parse failed: ${(e as Error).message}`}
	}

	const schemaResult = infoDictSchema.safeParse(parsed)
	if (!schemaResult.success) {
		return {name, passed: false, durationMs: r.durationMs, schemaIssue: schemaResult.error.issues[0]?.message ?? 'unknown schema issue'}
	}
	...
	return {name, passed: true, durationMs: r.durationMs, formatCount, title}
}
