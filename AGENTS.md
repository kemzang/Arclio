...
- **YtDlpJsRuntime** — Electron-as-Node runtime selector for yt-dlp. Probes `process.execPath` with `ELECTRON_RUN_AS_NODE=1`, requires Node >=22, builds `--no-js-runtimes --js-runtimes node:<process.execPath>`, and injects the runtime env only into the yt-dlp child-process tree. `src/main/services/ytDlpJsRuntime.ts`.
- **RuntimeSmoke** — packaged-app verification mode for Electron Node + managed yt-dlp + bundled `yt-dlp-ejs`. Runs from the packaged main process with `process.execPath`, rejects system PATH runtimes, remote EJS for managed binaries, Deno args, missing Node runtime, and missing bundled EJS. `src/main/runtimeSmoke.ts`.
...
- **GraphicsPolicy** — non-persisted runtime graphics gate from Electron GPU status/device info to renderer backdrop behavior. Keeps the saved backdrop preference separate from the effective hardware-only production decision; explicit dev software WebGL remains opt-in. `src/main/services/GraphicsPolicy.ts`, `src/shared/graphicsPolicy.ts`.
