...
- **慎重モード** — 設定可能なペース調整プリセット（*オフ · バランス · 慎重 · カスタム*）でリクエスト間に待ち時間を入れ、フラグメントスレッド数を制限して、大きなプレイリストで bot-blocks に遭う可能性を下げます
...
はい — 両方に対応しています。プレイリストまたはチャンネルの URL（例：`youtube.com/@handle`、`/channel/UC…`、`/c/Name`、`/user/Old`）を貼り付けると、Arroxy は最大 500 件を列挙し、そのあとリスト全体をキューに入れるか、個別の動画を選択できます。日付範囲と件数のフィルターは近日公開予定です。
...
| 機能    | 説明    |
| ---------------- | ---------------- |
| **プレイリスト・チャンネルのフィルター** | プレイリストまたはチャンネルを列挙する際の日付範囲と件数のフィルター（現在の上限は固定で 500 件） |
