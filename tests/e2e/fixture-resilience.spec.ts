...
test('Electron queue controls handle hold, priority, cancel, pause queue, and resume queue', async () => {
  ...
  await withFixtureProductApp(
    {
      behavior: { mediaSlowIds: [firstId, secondId, thirdId] },
      userDataPrefix: 'arroxy-fixture-controls-user-',
      outputPrefix: 'arroxy-fixture-controls-out-'
    },
    async ({ app, page, urls, queue, files }) => {
...
test('Electron metadata failure surfaces retry and recovers', async () => {
  ...
  await withFixtureProductApp(
    {
      behavior: { metadataFailureIds: [videoId] },
      userDataPrefix: 'arroxy-fixture-metadata-retry-user-',
      outputPrefix: 'arroxy-fixture-metadata-retry-out-'
    },
    async ({ page, fixtureServer, urls }) => {
...
test('Electron media failure can be retried to a completed output file', async () => {
  ...
  await withFixtureProductApp(
    {
      behavior: { mediaFailureIds: [videoId] },
      userDataPrefix: 'arroxy-fixture-media-retry-user-',
      outputPrefix: 'arroxy-fixture-media-retry-out-'
    },
...
test('Electron paused active fixture download resumes after app relaunch', async () => {
  ...
  await withFixtureProductApp(
    {
      behavior: { mediaSlowIds: [videoId] },
      userDataPrefix: 'arroxy-fixture-restart-user-',
      outputPrefix: 'arroxy-fixture-restart-out-'
    },
    async ({ page, queue, files, relaunch }) => {
      await prepareSingleConfirm(page, videoId);
      await page.locator('[data-testid="btn-download-now"]').click();
