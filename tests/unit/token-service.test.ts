...
function makeProvider(overrides: Partial<{ensureReady: () => Promise<void>; getVisitorData: () => Promise<string>; mintToken: (binding: string) => Promise<string>; releaseWindow: () => void; dispose: () => void}> = {}) {
	return {ensureReady: vi.fn().mockResolvedValue(undefined), getVisitorData: vi.fn().mockResolvedValue('visitor-abc'), mintToken: vi.fn().mockResolvedValue('token-xyz'), releaseWindow: vi.fn(), dispose: vi.fn(), ...overrides}
}
