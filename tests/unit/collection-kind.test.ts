...
describe('collectionKindForWizard', () => {
	...
	it('returns search for YouTube search results', () => {
		expect(collectionKindForWizard('https://www.youtube.com/results?search_query=arclio')).toBe('search')
	})
	...
	it('keeps the original channel intent when yt-dlp reports a playlist webpage URL', () => {
		expect(collectionKindForWizardUrls('https://www.youtube.com/@arclio/videos', 'https://www.youtube.com/playlist?list=PLtest')).toBe('channel')
	})
