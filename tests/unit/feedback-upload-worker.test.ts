...
describe('feedback upload Worker', () => {
	...
	it('stores valid gzip diagnostics in R2 and returns a report id', async () => {
		...
		const response = await worker.fetch(
			new Request('https://arclio.net/api/feedback-diagnostics', {
				method: 'POST',
	...
	it('rejects browser-style or oversized requests before writing to R2', async () => {
		const {bucket, put} = makeBucket()
		const missingMarker = await worker.fetch(new Request('https://arclio.net/api/feedback-diagnostics', {method: 'POST', headers: {'content-type': 'application/gzip', 'content-encoding': 'gzip'}, body: new Uint8Array([1])}), makeEnv(bucket))
		const tooLarge = await worker.fetch(
			new Request('https://arclio.net/api/feedback-diagnostics', {method: 'POST', headers: {'x-arclio-upload': 'feedback-diagnostic-v1', 'x-arclio-report-id': reportId, 'content-type': 'application/gzip', 'content-encoding': 'gzip', 'content-length': '1500001'}, body: new Uint8Array([1])}),
			{...makeEnv(bucket)}
		)
	...
	it('rate limits upload attempts before reading or writing diagnostics', async () => {
		...
		const response = await worker.fetch(
			new Request('https://arclio.net/api/feedback-diagnostics', {method: 'POST', headers: {'x-arclio-upload': 'feedback-diagnostic-v1', 'x-arclio-report-id': reportId, 'content-type': 'application/gzip', 'content-encoding': 'gzip', 'cf-connecting-ip': '203.0.113.10'}, body: new Uint8Array([31, 139, 8, 0])}),
			makeEnv(bucket, rateLimit)
		)
	...
	it('rejects invalid report ids before writing to R2', async () => {
		const {bucket, put} = makeBucket()
		const response = await worker.fetch(
			new Request('https://arclio.net/api/feedback-diagnostics', {method: 'POST', headers: {'x-arclio-upload': 'feedback-diagnostic-v1', 'x-arclio-report-id': 'report-123', 'content-type': 'application/gzip', 'content-encoding': 'gzip'}, body: new Uint8Array([31, 139, 8, 0])}),
			makeEnv(bucket)
		)
