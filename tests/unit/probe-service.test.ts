...
describe('ProbeService — video probe', () => {
  it('parses _type:video and derives isAudioOnlySource from extractor', async () => {
    const json = JSON.stringify({
      ...
      webpage_url: 'https://artist.bandcamp.com/track/x',
      formats: [{ format_id: 'mp3-128', ext: 'mp3', acodec: 'mp3', vcodec: 'none', abr: 128 }]
    });
      ...
      entries: [
        {
          ...
          thumbnails: [{ url: null }, { url: 'https://example.com/thumb.jpg' }]
        }
      ]
    });
    vi.mocked(spawnYtDlp).mockReturnValue(makeFakeProcessEmitting(json) as never);
...
describe('ProbeService — playlistMode arg threading', () => {
  it.each([
    ...
    const json = JSON.stringify({
      ...
      title: 't',
      extractor: 'youtube',
      formats: [{ format_id: 'mp4', ext: 'mp4', vcodec: 'avc1', acodec: 'aac', resolution: '720p' }]
    });
    vi.mocked(spawnYtDlp).mockReturnValue(makeFakeProcessEmitting(json) as never);
  ...
  it("default playlistMode='auto' adds neither --no-playlist nor --yes-playlist", async () => {
    const json = JSON.stringify({
      ...
      title: 't',
      extractor: 'youtube',
      formats: [{ format_id: 'mp4', ext: 'mp4', vcodec: 'avc1', acodec: 'aac' }]
    });
    vi.mocked(spawnYtDlp).mockReturnValue(makeFakeProcessEmitting(json) as never);
...
describe('ProbeService — _type: url redirect', () => {
  it('re-probes when first response is a url-redirect entry', async () => {
    ...
    const videoJson = JSON.stringify({
      ...
      extractor: 'generic',
      formats: [{ format_id: 'mp4-720', ext: 'mp4', vcodec: 'avc1', acodec: 'aac', resolution: '720p' }]
    });
