...
describe('preload diagnostics', () => {
  it('logs serialized preload-error events', () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Preload script failed', {
      preloadPath,
      error: expect.objectContaining({
        name: 'Error',
        message: 'preload exploded',
        stack: expect.stringContaining('preload exploded'),
  ...
  it('logs renderer load failures', () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Renderer failed to load', {
      errorCode: -6,
      errorDescription: 'ERR_FILE_NOT_FOUND',
      validatedURL: 'file:///app/out/renderer/index.html',
      isMainFrame: true,
      frameProcessId: 12,
      frameRoutingId: 34
    });
  ...
  it('forwards startup console warnings and errors', () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith('Renderer startup console warning', {
      level: 2,
      message: 'renderer warning',
      line: 10,
      sourceId: 'file:///renderer.js'
    });
    expect(logger.error).toHaveBeenCalledWith('Renderer startup console error', {
      level: 3,
  ...
  it('logs successful bridge probe details', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('Preload bridge verified', {
      preloadPath,
      bridge: {
        hasAppApi: true,
        hasPlatform: true,
  ...
  it('logs when the renderer finishes loading without window.appApi', async () => {
    ...
    expect(webContents.executeJavaScript).toHaveBeenCalledWith(expect.stringContaining('appApiKeys'));
    expect(logger.error).toHaveBeenCalledWith('Preload bridge missing after renderer load', {
      preloadPath,
      bridge: {
        hasAppApi: false,
        hasPlatform: false,
