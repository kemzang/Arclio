...
describe('cancelQueueBeforeExit', () => {
	...
	it('still disposes tokens and exits if queue cancellation throws', async () => {
		const deps = makeDeps()
		deps.queueService.cancel.mockRejectedValueOnce(new Error('boom'))
...
describe('finishShutdown', () => {
	it('disposes tokens, logs, and exits with code 0', () => {
		const deps = {tokenService: {dispose: vi.fn()}, logInfo: vi.fn(), exit: vi.fn()}
