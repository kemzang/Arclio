...
function makeIpcRenderer() {
  ...
  const ipcRenderer: PreloadIpcRenderer = {
    invoke: invoke as PreloadIpcRenderer['invoke'],
    on: on as PreloadIpcRenderer['on'],
    removeListener: removeListener as PreloadIpcRenderer['removeListener'],
    send: send as PreloadIpcRenderer['send']
  };
