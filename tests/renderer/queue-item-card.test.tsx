...
const actions = {cancelItemDownload: vi.fn(), pauseItemDownload: vi.fn(), resumeItemDownload: vi.fn(), removeQueueItem: vi.fn(), retryQueueItem: vi.fn(), openItemFolder: vi.fn(), openItemUrl: vi.fn(), setItemLane: vi.fn()}
...
describe('QueueItemCard — progress display', () => {
	it('shows progress bar and percent when running', () => {
		render(<QueueItemCard item={makeItem({status: 'running', lastJobId: 'job-1', progressPercent: 42.5, progressDetail: '1.2 MiB/s'})} />)
...
describe('QueueItemCard — phase indicators', () => {
	it.each([
		...
		const {container} = render(<QueueItemCard item={makeItem({status: 'running', lastJobId: 'job-1', progressPercent: 5, lastStatus: {key: statusKey as never, params: {}}})} />)
	...
	it('uses paused color class on the progress label when sleeping', () => {
		render(<QueueItemCard item={makeItem({status: 'running', lastJobId: 'job-1', progressPercent: 50, lastStatus: {key: 'sleepingBetweenRequests', params: {seconds: 5}}})} />)
	...
	it('uses brand color (no paused) when actively downloading', () => {
		render(<QueueItemCard item={makeItem({status: 'running', lastJobId: 'job-1', progressPercent: 50, lastStatus: {key: 'downloadingMedia', params: {}}})} />)
		const label = screen.getByTestId('queue-progress-label')
...
describe('QueueItemCard — subtitlesFailed warning', () => {
	it('shows the warning row when status=done and lastStatus=subtitlesFailed', () => {
		render(<QueueItemCard item={makeItem({status: 'done', finishedAt: '2026-04-27T10:30:00Z', lastStatus: {key: 'subtitlesFailed', params: {}}})} />)
	...
	it('does NOT show warning row when status=done and lastStatus=complete', () => {
		render(<QueueItemCard item={makeItem({status: 'done', finishedAt: '2026-04-27T10:30:00Z', lastStatus: {key: 'complete', params: {}}})} />)
	...
	it('does NOT show warning row when status=running and lastStatus=subtitlesFailed', () => {
		render(<QueueItemCard item={makeItem({status: 'running', lastJobId: 'job-1', lastStatus: {key: 'subtitlesFailed', params: {}}})} />)
		expect(screen.queryByTestId('queue-subs-warning')).not.toBeInTheDocument()
	})
})
